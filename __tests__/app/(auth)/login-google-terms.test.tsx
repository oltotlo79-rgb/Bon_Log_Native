/**
 * @module __tests__/app/(auth)/login-google-terms
 * LoginScreen の Google 規約同意フロー（TERMS_ACCEPTANCE_REQUIRED）結線テスト。
 * 既存の login-google-auth.test.tsx とは重複しない、モーダル表示・再試行・キャンセルのケースのみを扱う。
 *
 * useGoogleAuth の error 検知は「前回値との参照比較」で行われる（LoginScreen 実装）ため、
...
/**
 * LoginScreen を error: null でマウントしてから error を差し替えて rerender する。
...
function renderWithGoogleErrorTransition(error: Error) {
  ...
  const utils = renderWithProviders(<LoginScreen />);
...
describe('LoginScreen - Google 規約同意フロー', () => {
  ...
  it('モーダル表示中はボタン下の生エラー表示が隠れる', () => {
    renderWithGoogleErrorTransition(termsRequiredError);
    // FormErrorMessage には showGoogleTermsModal=true のとき null を渡す実装のため、
    // ERR_TERMS_ACCEPTANCE_REQUIRED はモーダル本文の 1 箇所にのみ表示される。
  ...
  describe('モーダル内での再送 403（modalOpenedForError による区別）', () => {
    ...
    it('キャンセルして閉じた後に再び 403 が起きた場合、新しい 403 は初回として扱われモーダル内にエラーが表示されない', () => {
      ...
      const secondFirstTimeError = new ApiError({
        code: 'TERMS_ACCEPTANCE_REQUIRED',
        status: 403,
        message: '2回目のログイン試行での初回 403（テスト用）',
      });
