...
describe('PostCard', () => {
  ...
  describe('基本表示', () => {
    it('ユーザーのニックネームが表示される', () => {
      render(<PostCard {...makePostCardProps()} />);
      expect(screen.getByText('松の匠')).toBeTruthy();
    });

    it('投稿本文が表示される', () => {
      render(<PostCard {...makePostCardProps({ content: '黒松の春管理' })} />);
  ...
  describe('content=null の防衛表示', () => {
    it('content=null のとき「(内容がありません)」が表示される', () => {
      render(<PostCard {...makePostCardProps({ content: null })} />);
      expect(screen.getByText('(内容がありません)')).toBeTruthy();
    });

    it('content=undefined のとき「(内容がありません)」が表示される', () => {
      render(<PostCard {...makePostCardProps({ content: undefined })} />);
      expect(screen.getByText('(内容がありません)')).toBeTruthy();
    });
  });
  ...
  describe('disableNavigation=false（デフォルト）', () => {
    it('カードをタップすると投稿詳細画面へ遷移する', () => {
      render(<PostCard {...makePostCardProps({ id: 'post-xyz' })} />);
  ...
  describe('disableNavigation=true（投稿詳細画面）', () => {
    it('カードをタップしても遷移しない', () => {
      render(<PostCard {...makePostCardProps({ disableNavigation: true })} />);
      fireEvent.press(screen.getByTestId('post-card'));
  ...
  describe('画像・ジャンル', () => {
    it('media が空のとき画像エリアが表示されない', () => {
      render(<PostCard {...makePostCardProps({ media: [] })} />);
    ...
    it('genres が空のとき genre タグが表示されない', () => {
      render(<PostCard {...makePostCardProps({ genres: [] })} />);
