...
- Node.js LTS / npm
- EAS CLI（ビルド・提出時）
- Android: Android Studio + エミュレータ（開発の基本。リリース対象 — 計画書 §1.5）
- iOS 互換確認を行う場合のみ: macOS + Xcode（リリースは Android 先行）
...
npm install
npx expo run:android               # development build 作成 + 起動
# 以後の開発は
npx expo start
```
...
- **書き込みを伴う開発・検証はローカルサーバー推奨**（本番 DB を汚さないため）:
  ...
  - 実機: PC の LAN IP
  - iOS シミュレータ（互換確認時のみ・要 macOS）: `http://localhost:3000`
...
- 配布: EAS Build → Play 内部テスト（計画書 Phase 5。TestFlight は将来の iOS リリース時）
