/**
 * @module app/settings/muted/index
 * ミュート中ユーザー一覧画面（ugc-safety.md §6.2）。
 * store-compliance.md 要件: ミュート解除導線をアプリ内に露出する。
 */
...
export default function SettingsMutedScreen() {
  ...
  const mutedUsers = data?.pages.flatMap((page) => page.items) ?? [];
  ...
  const handleUnmute = useCallback(
    ...
    [isOffline, unmuteMutation, showToast]
  );
  ...
  const renderItem = useCallback(
    ({ item }: ListRenderItemInfo<UserMinimalWithBio>) => (
      <MutedUserListItem
        ...
        isUnmuting={unmutingIds.has(item.id)}
      />
    ),
  ...
  if (isError) {
    ...
    return (
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        <ScreenError
          title="読み込めませんでした"
          description={isOfflineError ? ERR_OFFLINE_ACTION : ERR_MUTE_LIST_LOAD_FAILED}
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      {mutedUsers.length === 0 ? (
        <ScreenEmpty
          ...
          description="ミュートしたユーザーがここに表示されます"
        />
      ) : (
        <FlatList
          data={mutedUsers}
          ...
          ListFooterComponent={
            ...
            ) : !hasNextPage && mutedUsers.length > 0 ? (
