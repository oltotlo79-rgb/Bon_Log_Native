...
`like`（いいね）→ `comment`（コメント）→ `reply`（返信）→ `comment_like`（コメントへのいいね）→ `follow`（フォロー）→ `quote`（引用）→ `follow_request`（フォローリクエスト）→ `follow_request_approved`（フォローリクエストの承認）→ `mention`（メンション）→ `message`（メッセージ）→ `repost`（リポスト）。
...
| 属性 | 値 |
|------|---|
| 行の高さ | 最小 52pt（`minHeight: 52` / `paddingVertical: 12pt` / `paddingHorizontal: spacing4`） |
...
| 2 | `comment` | コメント |
| 3 | `reply` | 返信 |
| 4 | `comment_like` | コメントへのいいね |
| 5 | `follow` | フォロー |
| 6 | `quote` | 引用 |
| 7 | `follow_request` | フォローリクエスト |
| 8 | `follow_request_approved` | フォローリクエストの承認 |
| 9 | `mention` | メンション |
| 10 | `message` | メッセージ |
| 11 | `repost` | リポスト |
...
### 4.2 `message` の扱い（副ラベルなし）

> **改訂注記（2026-07-13）:** 旧仕様は「`message` は MVP スコープ外につき、行のラベル下に『近日公開予定』の補足テキスト（`sublabel`）を表示することを推奨」としていたが、実装済みの `NotificationTypeSettings.tsx` はどの行にも `sublabel` を渡していない。`message` は他の 10 種と全く同じ見た目・挙動のトグル行として表示され、「近日公開予定」等の特別な注記は一切ない。

- `NotificationToggleRow` は `sublabel?: string` という補足テキスト用の任意 prop を持つが、`NotificationTypeSettings.tsx` はこの prop を渡していない（=現状、どのキーにも sublabel は表示されない）
- `message` タイプの通知設定トグル自体は、DM（ダイレクトメッセージ）機能がこのリポジトリの MVP スコープ外（CLAUDE.md MVP スコープに `message` 系機能は含まれない）であるかどうかに関わらず、Web との通知設定パリティのために表示される。トグルを ON/OFF してもサーバー側の設定値が変わるのみで、アプリ内に DM 機能そのものが存在しなくても副作用はない
- 将来 `sublabel` を使う別の理由（例: 別キーの説明文表示）が生じた場合に備え、プロパティ自体は実装から削除しない
...
**（2026-07-13 修正）:** 旧仕様にあった「グループ見出し行 × 3」のスケルトンは削除した（グループ見出し自体が存在しないため）。
...
| `sublabel` | `string \| undefined` | 補足テキスト（任意。現状はどのキーにも渡されていない。§4.2 参照）|
...
| sublabel（補足。現状未使用）| `textXs`（10pt）/ `colorTextTertiary` |
...
| Web `components/settings/NotificationPreferences.tsx` | 通知キーの並び順・11 種構成・グループ見出しなしのフラット表示はこの Web 実装に準拠する |
...
| 各トグル行に説明文（description）を表示するか | Web の `NotificationPreferences.tsx` はラベルの下に一行説明（例:「投稿にいいねされた時」）を表示しているが、Native の現行実装はラベルのみで説明文を表示していない。Web とのコピー粒度差として残っている | PM（要判断）|  未解決（新規発見。2026-07-13 調査で判明）|
