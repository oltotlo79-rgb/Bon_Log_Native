...
同日追記（2026-07-13・エラー色トークンの是正）: §0.2・§0.3・§0.9.5 の `colorError` 記載値を `#c0392b` から `#c21721` へ修正した。根拠・是正対象コードの一覧は `design-tokens.md` §11 を参照
対象画面:
- `(auth)/login` — ログイン
- `(auth)/register` — 新規登録
...
- テキストスタイル: `textSm`（12pt）/ `colorError`（`#c21721`）
...
**（2026-07-13 修正）:** 上記 2 箇所の `colorError` の値表記を `#c0392b` から `#c21721` に修正した。旧稿の `#c0392b` は Web の `--destructive`/`--aka`（`oklch(0.52 0.20 26)`）の不正確な近似値であり、実装 `lib/constants/design-tokens.ts` の実際の値（`#c21721`）とは異なっていた。判定根拠・是正対象コード一覧は `design-tokens.md` §11 を参照。
...
| `colorError`（`#c21721`）on `colorErrorBg`（`#fdf0ef`）| — | 約5.5:1（AA）|

**（2026-07-13 修正）:** `colorError` の値表記を `#c0392b` から `#c21721` に、コントラスト比を実装値での再計算値（約5.5:1）に修正した。根拠は `design-tokens.md` §11 を参照。
...
- 「利用規約」「プライバシーポリシー」はインラインのテキストリンク / `colorTextLink` / underline
