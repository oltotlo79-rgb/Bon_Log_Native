...
追記（2026-07-14・実装トークン全体監査）: 本書に値を記載しているトークンを `lib/constants/design-tokens.ts` と再照合し、§2.2・§2.3・§5・§7・§10 を現行実装に同期した。余白・角丸・字間・影・アニメーションは一致しており変更なし。直接比較できない Web 専用値・コンポーネント固有値・実装専用トークンの扱いも各節に明記した。

対象: `lib/constants/design-tokens.ts`（core が TypeScript 定数として実装）
前提: StyleSheet + 定数ベース（NativeWind 採否確定まで）

照合方針（2026-07-14）: 本書と実装に同名トークンがある場合は実装値を正とする。React Native の数値へ本書で `pt`、duration の数値へ `ms` を付けた表記は同値として扱う。Web 専用トークン、コンポーネント内の固有定数、本書に値を掲載していない実装専用トークンは `design-tokens.ts` との直接比較対象外とする。
...
| `--foreground` | oklch(0.12 0 0) | `#060606` | 本文テキスト（深墨） |
| `--card` | oklch(0.99 0 0) | `#fcfcfc` | カード背景（ほぼ白） |
| `--card-foreground` | oklch(0.12 0 0) | `#060606` | カード内テキスト |
| `--primary` | oklch(0.20 0 0) | `#161616` | 主要アクション色（墨） |
| `--primary-foreground` | oklch(1 0 0) | `#ffffff` | プライマリボタンテキスト |
| `--secondary` | oklch(0.94 0 0) | `#ebebeb` | セカンダリ背景 |
| `--secondary-foreground` | oklch(0.20 0 0) | `#161616` | セカンダリテキスト |
| `--muted` | oklch(0.96 0 0) | `#f2f2f2` | 薄いサポート背景 |
| `--muted-foreground` | oklch(0.40 0 0) | `#484848` | 補助テキスト（日時・カウント等） |
| `--accent` | oklch(0.92 0 0) | `#e4e4e4` | アクセント背景（選択状態等） |
...
| `--border` | oklch(0.85 0 0) | `#cececd` | ボーダー |
| `--input` | oklch(0.90 0 0) | `#d9d9d9` | 入力フィールドの枠 |
| `--ring` | oklch(0.30 0 0) | `#2e2e2e` | フォーカスリング |
| **ブランドカラー** | | | |
| `--bonsai-green` | oklch(0.35 0 0) | `#3a3a3a` | FAB・アクセント（Web ではグレーに近い墨緑）|
...
| `--washi` | oklch(0.98 0 0) | `#f8f8f8` | 和紙白（ナビ背景等） |
| `--sumi` | oklch(0.18 0 0) | `#121212` | 墨色（ナビアイコン等） |
...
**直接比較できない値:** この表は Web 名からモバイル値への対応表であり、同名 export の一覧ではない。実装コメントから対応先が明確な行は現行トークン値へ同期した。一方、`--accent-foreground`、`--input`、`--bonsai-brown`、`--bonsai-beige`、`--bonsai-cream`、`--matcha`、`--kincha` には `design-tokens.ts` 内に一対一の対応トークンがないため、各モバイル hex は Web 移植時の参考値であり、実装との直接比較はできない。

**注意:** bonsai-green は Web では彩度ゼロの墨色として定義されている（oklch(0.35 0 0)）。
「緑」の印象はブランド名による連想であり、実際の色値はダーク寄りのグレー。
モバイルでも同じ値を使うが、アイコン・FAB に使う際は `#3a3a3a` の濃いグレーとして扱う。

### 2.3 セマンティックカラー（モバイル用途別）

以上の基本値を用途ごとに命名したセマンティック層を定義する。
`lib/constants/design-tokens.ts` の現行実装はこの命名を使う。本節の値は実装を正として同期する。
...
| `colorSurfaceWashi` | `#f8f8f8` | ナビバー・ヘッダー |
| `colorSurfaceKinoko` | `#f0ece4` | ランディング的な温かみある背景（任意で使用） |
| `colorSurfaceMuted` | `#f2f2f2` | 非アクティブ・ローディングスケルトン |

#### テキスト系

| トークン名 | hex 値 | 用途 |
|-----------|--------|------|
| `colorTextPrimary` | `#060606` | 本文・見出し |
| `colorTextSecondary` | `#484848` | 補助テキスト（日時・カウント・プレースホルダー） |
| `colorTextTertiary` | `#8a8a8a` | さらに薄い補助（キャプション・ヒント） |
| `colorTextInverse` | `#ffffff` | 墨色背景上のテキスト |
| `colorTextLink` | `#161616` | リンク・ハッシュタグ（Web では primary = 墨） |
| `colorTextHashtag` | `#3a3a3a` | ハッシュタグ専用（Web: text-bonsai-green） |

#### ボーダー・セパレータ系

| トークン名 | hex 値 | 用途 |
|-----------|--------|------|
| `colorBorder` | `#cececd` | 汎用ボーダー |
| `colorBorderLight` | `#e4e4e4` | 薄いセパレータ |
...
| トークン名 | hex 値 | 用途 |
|-----------|--------|------|
| `colorActionPrimary` | `#161616` | 主要ボタン背景（墨） |
| `colorActionPrimaryText` | `#ffffff` | 主要ボタンテキスト |
| `colorActionSecondary` | `#ebebeb` | セカンダリボタン背景 |
| `colorActionSecondaryText` | `#161616` | セカンダリボタンテキスト |
| `colorFab` | `#3a3a3a` | FAB 背景（Web: bg-bonsai-green） |
...
| `colorWarning` | `#b45309` | 警告（Web: ConfirmDialog の amber-700） |
| `colorWarningBg` | `#fdf8e1` | 警告バックグラウンド |
| `colorInfo` | `#2c6e8a` | 情報・ヒント |
| `colorInfoBg` | `#e8f4f8` | 情報バックグラウンド |

**比較上の注意:** success / warning / info の hex 値は Web の oklch 定義には存在しないため、Web トークンとの直接比較はできない。本書では `design-tokens.ts` の実装値を正として記載する。
...
| トークン名 | hex 値 | 用途 |
|-----------|--------|------|
| `colorNavBackground` | `#f8f8f8` | ボトムナビ背景（Web: bg-washi/90） |
| `colorNavIconActive` | `#121212` | アクティブタブアイコン（Web: text-sumi） |
| `colorNavIconInactive` | `#12121266` | 非アクティブタブアイコン（Web: text-sumi/40） |
| `colorNavLabel` | `#121212` | アクティブラベル |
| `colorNavLabelInactive` | `#12121266` | 非アクティブラベル |
...
モバイル実装ではプラットフォーム別のシステムフォント名ではなく、Shippori Mincho の太さ別フォントファミリーをトークン化して使用する。

| トークン名 | 実装値 | 用途 |
|-----------|--------|------|
| `fontFamilySerif` | `ShipporiMincho_400Regular` | 本文・通常 UI |
| `fontFamilySerifMedium` | `ShipporiMincho_500Medium` | 中見出し・軽い強調 |
| `fontFamilySerifBold` | `ShipporiMincho_700Bold` | 大見出し・強い強調 |

フォントがアプリ起動時に実際にロードされるか、未ロード時にどの OS フォントへフォールバックするかは `design-tokens.ts` の値だけでは直接比較・検証できない。

### 5.2 テキストスケール

| トークン名 | fontSize | fontFamily（対応ウェイト） | lineHeight | 用途 |
|-----------|----------|----------------------------|-----------|------|
| `textXs` | 10pt | `fontFamilySerif`（400） | 14pt | タブラベル・バッジ |
| `textSm` | 12pt | `fontFamilySerif`（400） | 18pt | 補助テキスト・タイムスタンプ |
| `textBase` | 14pt | `fontFamilySerif`（400） | 22pt | 投稿本文・一般テキスト |
| `textMd` | 15pt | `fontFamilySerif`（400） | 23pt | リスト項目・ナビラベル |
| `textLg` | 17pt | `fontFamilySerifMedium`（500） | 24pt | セクション見出し・ユーザー名 |
| `textXl` | 20pt | `fontFamilySerifBold`（700） | 28pt | ページタイトル |
| `text2xl` | 24pt | `fontFamilySerifBold`（700） | 32pt | 大見出し |

`fontSize`・`lineHeight`・`fontFamily` は実装と直接比較して一致を確認した。実装の `text*` オブジェクトは `fontWeight` プロパティを持たず太さ別 `fontFamily` で表現するため、従来表の `fontWeight` 値とはプロパティ単位の直接比較ができない。
...
|---------------|------------|-------------------|------|
| `#060606` on `#ffffff` | — | 20.3:1 | AA / AAA 達成 |
| `#060606` on `#fcfcfc` | — | 19.8:1 | AA / AAA 達成 |
| `#060606` on `#f2f2f2` | — | 18.1:1 | AA / AAA 達成 |
| `#484848` on `#ffffff` | — | 9.2:1 | AA / AAA 達成 |
| `#484848` on `#fcfcfc` | — | 8.9:1 | AA / AAA 達成 |
| `#484848` on `#f2f2f2` | — | 8.2:1 | AA / AAA 達成 |
| `#8a8a8a` on `#ffffff` | — | 3.5:1 | AA 未達（大テキストのみ AA） |
| `#ffffff` on `#161616` | — | 18.1:1 | AA / AAA 達成（プライマリボタン） |
| `#ffffff` on `#3a3a3a` | — | 11.4:1 | AA / AAA 達成（FAB） |
...
- **success / warning / info の Web 整合:** モバイルの値は §2.3 記載どおり実装済み。対応する Web oklch トークンがないため、Web の toast・badge との視覚的整合は `design-tokens.ts` だけでは直接比較できない。
- **フォントの実行時検証:** 太さ別 Shippori Mincho のトークン値は §5.1 記載どおり実装済み。フォントロードと OS ごとのフォールバック結果は実機で別途確認する。
...
**本監査で直接比較できない範囲:** §8 の `FAB_SIZE`、BottomTabBar 高さ、Avatar サイズ等はコンポーネント固有値であり `design-tokens.ts` の export ではない。また、実装にのみ存在し本書が値を掲載していない活性レベル・カテゴリ・図鑑・農薬・オーバーレイ系カラーおよび `easingBounceRN` は、比較元となる文書値がないため今回の値一致判定には含めない。
