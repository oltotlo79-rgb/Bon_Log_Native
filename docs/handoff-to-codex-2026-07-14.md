...
### 10.1 Google Play 公開前監査と Native 追加対応（2026-07-14 続行分）

§10 の続行として、Android 公開前の課金・認証・Push・監視・ビルド設定を横断監査した。以下は Native 側で実装済みである。

実装コミットは `2e0eac6`（課金・Push・削除）と `b305e2e`（Android release・Sentry・依存）である。

- RevenueCat の購入・復元直前にサーバーの `currentUser.id` で再 identify し、billing wrapper 経由の App User ID が完全一致した場合だけ SDK を呼ぶ。不一致・identify 失敗は共通文言で中断し、SDK 由来の生エラーを UI に出さない。`success` のときだけ `users.me` を invalidate し、2秒間隔・最大5回の反映確認、timeout、手動再確認、unmount cleanup を追加した
- アカウント削除 API が失敗した場合は sign out せず、セッションと再試行 UI を維持する。サーバー削除成功時だけローカル撤収する
- ログイン時の Push 自動登録は、既に通知許可済みの場合だけ行い、OS 許可ダイアログを自動表示しない。ログアウト開始時の登録世代無効化、effect cleanup、進行中登録の完了待ちを追加し、遅延した登録が unregister 後に残る競合を防いだ。device token DELETE の二重 URL encode も解消した
- Sentry は `sendDefaultPii: false` を明示し、request、headers、cookies、body、query、URL、user、contexts、extra、breadcrumbs、exception/message を再帰 scrub する。Bearer / Basic / JWT / email、UGC本文、検索語、URL fragment、fetch/XHR breadcrumb 内 URL を除去し、循環参照・深さ・件数上限も設けた
...
1. Androidから到達するlegal / helpからStripe・カード・Web固定価格・未確認trial誘導を除き、Android専用Terms / Privacy / Helpを用意する
2. 未知Google Userの新規作成時だけ`termsAccepted / termsVersion`を必須にし、server時刻の同意証跡を保存する。既存Google loginは壊さない
3. Stripe / RevenueCatをprovider別entitlementとして保存し、有効権利のORをサーバー`isPremium`の正とする
4. Block / blockedBy / muteの相互関係にあるactorの既存通知を一覧・未読数からDB query段階で除外する
5. account deletion時の全R2媒体をtransactional outbox等でdurableに削除・再試行・dead-letter可能にする
6. PrivacyをNative実SDKとデータフローへ合わせ、Play Consoleへ転記できるData Safety worksheetを返す

cfw は引き続き読み取り専用で、Native 側から変更していない。cfw handback後はOpenAPI再生成、Android専用URL結線、Google同意付き新規登録UIとtyped error処理、Play sandbox結合確認が必要である。

**最終品質ゲート:**

- `npx expo-doctor`: 21/21成功
- `npm run generate:api`: 成功し、生成型に差分なし
- `npx tsc --noEmit`: 成功
- `npm run lint`: エラー0 / 既存警告139。今回変更ファイルに新規警告なし
...
- ブランチ: `main`（`origin/main` と同期済み・up to date）
- 計測時点のコミット: `a083a43126e37300899aaafc4769a44f01913c81`（`refactor: UI層のローカル重複定義をlib共通実装へ差し替え`）。この文書自身のコミットは計測後に作成されるため、`git log -1` の実際の HEAD はこれより新しくなる（構造上の制約であり実害なし）
