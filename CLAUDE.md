...
- **現状: アプリ本体は実装済みで、公開前の Web 準拠差分解消・実機 QA・ストア準備段階。** 2026-07-14 時点の品質ゲートと残課題は `docs/handoff-to-codex-2026-07-14.md` を参照する。以後の変更もすべて以下の規約に従う

当初計画の MVP スコープ: フィード / 投稿 CRUD / コメント / いいね / フォロー / 通知 / プロフィール / 検索
（図鑑・農薬・肥料・ホルモン・分析・admin・ショップ・イベントは当初 MVP のスコープ外 — 計画書 §10）。現在の公開方針は Web 版の全機能を Web 準拠で実装しきってから公開することであり、実装済み範囲と残差分は上記の引き継ぎ文書を正とする
