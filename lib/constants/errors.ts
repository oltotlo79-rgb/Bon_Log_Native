...
/** 設定画面の 2FA 有効化コードが正しくない（ログイン経路の ERR_2FA_INVALID_CODE とは異なり、その場での再入力を促す） */
export const ERR_2FA_ENABLE_INVALID_CODE =
  '認証コードが正しくありません。もう一度お試しください。';

/** 設定画面の 2FA セットアップ情報の期限切れ（TTL 経過後に enable を実行） */
export const ERR_2FA_SETUP_EXPIRED_SETTINGS =
  '設定の有効期限が切れました。最初からやり直してください。';

/** 2FA が既に有効化済みの状態で setup/enable を再実行しようとした場合 */
export const ERR_2FA_ALREADY_ENABLED = '2段階認証はすでに有効になっています。';

/** 2FA 無効化時のパスワード検証失敗 */
export const ERR_2FA_DISABLE_INCORRECT_PASSWORD = 'パスワードが正しくありません。';

/** 2FA が有効化されていない状態で disable/regenerate を実行しようとした場合 */
export const ERR_2FA_NOT_ENABLED = '2段階認証は有効になっていません。';
...
// ---------------------------------------------------------------------------
// パスワード変更・メールアドレス変更系（設定画面）
// ---------------------------------------------------------------------------

/** パスワード変更時の現在のパスワード不一致（401 AUTH_INVALID_CREDENTIALS） */
export const ERR_PASSWORD_CHANGE_INVALID_CURRENT = '現在のパスワードが正しくありません。';

/** パスワード変更時の新しいパスワードの強度不足（400 VALIDATION_ERROR） */
export const ERR_PASSWORD_CHANGE_WEAK =
  '新しいパスワードの強度が不足しています。8文字以上でアルファベットと数字を両方含めてください。';

/** パスワード未設定の OAuth 専用アカウントによるパスワード変更不可（409 CONFLICT） */
export const ERR_PASSWORD_CHANGE_OAUTH_ONLY =
  'このアカウントは外部サービス連携でログインしているため、パスワードの変更はできません。';

/** メールアドレス変更リクエスト時の現在のパスワード不一致（401 AUTH_INVALID_CREDENTIALS） */
export const ERR_EMAIL_CHANGE_INVALID_PASSWORD = '現在のパスワードが正しくありません。';

/** パスワード未設定の OAuth 専用アカウントによるメールアドレス変更不可（409 CONFLICT） */
export const ERR_EMAIL_CHANGE_OAUTH_ONLY =
  'このアカウントは外部サービス連携でログインしているため、メールアドレスの変更はできません。';
...
/**
 * 盆栽園登録の住所ジオコーディングで住所が解決できない場合（404 NOT_FOUND）。
 * 文言は Web 版 Bon_Log_cfw/lib/constants/messages.ts の MSG_ADDRESS_NOT_FOUND と揃える。
 */
...
/** URL が http(s) スキームで始まっていない（イベント外部リンク・盆栽園 Web サイト等の共通検証） */
export const ERR_URL_INVALID_PROTOCOL = 'URLは https:// または http:// から始めてください。';
...
/** ダイレクトメッセージ送信失敗 */
export const ERR_MESSAGE_SEND_FAILED = 'メッセージの送信に失敗しました。もう一度お試しください。';

/** ダイレクトメッセージ送信の短時間レート制限 */
export const ERR_MESSAGE_RATE_LIMITED =
  'メッセージの送信が多すぎます。しばらく待ってから再試行してください。';

/** ダイレクトメッセージの日次送信上限 */
export const ERR_MESSAGE_DAILY_LIMIT =
  '本日のメッセージ送信上限に達しました。明日またお試しください。';

/** ブロック・参加状態変更等により会話へ送信できない */
export const ERR_MESSAGE_UNAVAILABLE = 'この会話にはメッセージを送信できません。';
...
/** 投票操作失敗 */
export const ERR_POLL_VOTE_FAILED = '投票に失敗しました。もう一度お試しください。';

/** 投票済みのアンケートに再投票しようとした場合（409 CONFLICT） */
export const ERR_POLL_ALREADY_VOTED = '既に投票済みです。';

/** 終了済みアンケートへの投票操作（403 / 400） */
...
/** イベント終了日時が開始日時より前（送信前バリデーション） */
export const ERR_EVENT_END_DATE_BEFORE_START = '終了日時は開始日時以降にしてください。';
...
/** 緯度が有効範囲外（送信前バリデーション・テンプレート） */
export const ERR_SHOP_LATITUDE_OUT_OF_RANGE = (min: number, max: number) =>
  `緯度は ${min}〜${max} の数値を入力してください。`;

/** 経度が有効範囲外（送信前バリデーション・テンプレート） */
export const ERR_SHOP_LONGITUDE_OUT_OF_RANGE = (min: number, max: number) =>
  `経度は ${min}〜${max} の数値を入力してください。`;
...
/** 公開予定日時が未選択（送信前バリデーション） */
export const ERR_SCHEDULED_POST_DATE_REQUIRED = '公開予定日時を選択してください。';

/** 公開予定日時が現在時刻以前（送信前バリデーション） */
export const ERR_SCHEDULED_POST_DATE_NOT_FUTURE = '公開予定日時は現在より未来に設定してください。';

/** 公開予定日時が許容日数を超過（送信前バリデーション・テンプレート） */
export const ERR_SCHEDULED_POST_DATE_TOO_FAR = (days: number) =>
  `公開予定日時は${days}日以内に設定してください。`;

/** 予約投稿作成時に pending 上限超過または日時が無効（400 VALIDATION_ERROR・テンプレート） */
export const ERR_SCHEDULED_POST_PENDING_LIMIT_REACHED = (limit: number) =>
  `保留中の予約投稿が上限（${limit}件）に達しているか、日時が無効です。`;

/** 予約投稿編集は pending 状態のみ許可（400 VALIDATION_ERROR） */
export const ERR_SCHEDULED_POST_EDIT_PENDING_ONLY = 'pending 状態の投稿のみ編集できます。または日時が無効です。';
...
/**
 * useChangePasswordMutation 専用のエラーメッセージ変換。
 * AUTH_INVALID_CREDENTIALS（現パスワード不一致）/ CONFLICT（OAuth 専用アカウント）/
 * VALIDATION_ERROR（新パスワード強度不足）をそれぞれの文脈に応じた文言へ対応付ける。
 */
export function messageForChangePasswordError(code: MobileApiErrorCode): string {
  if (code === 'AUTH_INVALID_CREDENTIALS') return ERR_PASSWORD_CHANGE_INVALID_CURRENT;
  if (code === 'CONFLICT') return ERR_PASSWORD_CHANGE_OAUTH_ONLY;
  if (code === 'VALIDATION_ERROR') return ERR_PASSWORD_CHANGE_WEAK;
  if (code === 'RATE_LIMITED') return ERR_RATE_LIMIT;
  return ERR_GENERIC;
}

/**
 * useRequestEmailChangeMutation 専用のエラーメッセージ変換。
 * 列挙攻撃対策により newEmail 重複は常に 200 のため、ここでは
 * AUTH_INVALID_CREDENTIALS（現パスワード不一致）と CONFLICT（OAuth 専用アカウント）のみ考慮する。
 */
export function messageForEmailChangeRequestError(code: MobileApiErrorCode): string {
  if (code === 'AUTH_INVALID_CREDENTIALS') return ERR_EMAIL_CHANGE_INVALID_PASSWORD;
...
/** ダイレクトメッセージ送信 API のエラーコードを操作文脈に合う文言へ変換する。 */
export function messageForSendMessageError(code: MobileApiErrorCode): string {
  if (code === 'RATE_LIMITED') return ERR_MESSAGE_RATE_LIMITED;
  if (code === 'VALIDATION_ERROR') return ERR_MESSAGE_DAILY_LIMIT;
  if (code === 'NOT_FOUND') return ERR_MESSAGE_UNAVAILABLE;
